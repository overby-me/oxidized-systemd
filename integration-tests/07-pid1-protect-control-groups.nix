{
  ...
  patchScript = ''
    ...
    # Skip testcase_delegate_subgroup — requires Delegate=yes + DelegateSubgroup= (not yet implemented)
    sed -i '/^testcase_delegate_subgroup()/,/^}/d' TEST-07-PID1.protect-control-groups.sh
  '';
}
