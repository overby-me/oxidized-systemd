{
  ...
  patchScript = ''
    ...
    [Service]
    Type=notify
    ExecStart=bash -c "systemd-notify --ready; systemd-notify RELOADING=1; sleep 60; exit 1"
    Restart=on-abort
    EOF
    ...
    [Service]
    Type=notify
    ExecStart=bash -c 'systemd-notify --ready; sleep 2; systemd-notify RELOADING=1; sleep 1; systemd-notify --ready; sleep 60'
