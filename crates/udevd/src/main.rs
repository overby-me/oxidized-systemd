//! systemd-udevd — Device manager daemon.
//!
//! This daemon manages device nodes in `/dev/`, listens for kernel uevents
//! via netlink, matches them against udev rules, and executes configured
//! actions (creating symlinks, setting permissions, running programs, etc.).
//!
//! Features:
...
fn recv_uevent(fd: i32) -> Option<UEvent> {
...
fn discover_rules_files() -> Vec<PathBuf> {
...
fn parse_rules_file(path: &Path) -> io::Result<Vec<Rule>> {
...
fn device_db_path(event: &UEvent) -> PathBuf {
        ...
        while i < argv.len() {
            match argv[i].as_str() {
                ...
                "-N" | "--resolve-names" => {
                    i += 1;
                    if i < argv.len() {
                        args.resolve_names = argv[i].clone();
                    }
                }
                "-c" | "--children-max" => {
                    i += 1;
                    if i < argv.len() {
                        args.children_max = argv[i].parse().unwrap_or(MAX_WORKERS);
                    }
                }
                "-e" | "--exec-delay" => {
                    i += 1;
                    if i < argv.len() {
                        args.exec_delay = argv[i].parse().unwrap_or(0);
                    }
                }
                "-t" | "--event-timeout" => {
                    i += 1;
                    if i < argv.len() {
                        args.event_timeout = argv[i].parse().unwrap_or(EVENT_TIMEOUT_SECS);
                    }
                }
                "--version" => {
