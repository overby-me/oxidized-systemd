...
impl TimesyncDaemon {
    ...
    fn run(&mut self) {
        ...
        let mut dbus_next_retry = std::time::Instant::now();
        ...
        loop {
            ...
            if _dbus_conn.is_none() && std::time::Instant::now() >= dbus_next_retry {
                match setup_dbus(self.shared_state.clone()) {
                    ...
                    Err(e) => {
                        ...
                        dbus_next_retry = std::time::Instant::now() + DBUS_RETRY_INTERVAL;
                    }
                }
            }
