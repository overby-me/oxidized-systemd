...
fn generate(config: &CmdlineConfig) -> GeneratedFiles {
    ...
    let unbound_routes: Vec<&RouteConfig> = routes_by_device.get("").cloned().unwrap_or_default();
...
mod tests {
    ...
    fn test_parse_ifname_policy_invalid() {
        ...
        assert!(
            parse_cmdline("net.ifname_policy=aa:bb:cc:dd:ee:ff")
                .ifname_policies
                .is_empty()
        );
        // MAC not last -> dropped.
        assert!(
            parse_cmdline("net.ifname_policy=aa:bb:cc:dd:ee:ff,path")
                .ifname_policies
                .is_empty()
        );
        // Malformed MAC (and not a policy) -> dropped.
        assert!(
            parse_cmdline("net.ifname_policy=path,bogus")
                .ifname_policies
                .is_empty()
        );
    }
    ...
    fn test_generate_ifname_policy_default() {
        let config = parse_cmdline("net.ifname_policy=keep,kernel,path");
        let files = generate(&config);
        let content = files.files.get("72-default.link").expect("72-default.link");
        assert_eq!(
            content,
            "# Automatically generated by systemd-network-generator\n\
             \n\
