...
/// [`drive_run_queue`] owns activation, completion, requeue and timeouts), then
/// blocks until the registry drains so the caller keeps the synchronous
/// activation contract. A generous overall deadline plus a periodic wake bound
/// the wait on top of the per-job timeouts. Errors are surfaced per job by the
/// drive, so the returned vector is always empty; the signature matches
/// `activate_needed_units` for a drop-in swap at the call site.
pub fn activate_needed_units_via_job_graph(
    target_id: UnitId,
    run_info: ArcMutRuntimeInfo,
) -> Vec<UnitOperationError> {
    use crate::units::jobs::{JobMode, JobKind};
    ...
    // overall deadline is a boot-wide safety net above the per-job timeouts; the
    // periodic wake guarantees the drive re-scans even if no other event fires.
    ...
    info!("activate_needed_units_via_job_graph: {} closure settled", target_id.name);
    Vec::new()
}
