...
| 26-SYSTEMCTL | `edit --global` mask removed 2026-07-27; interactive EDITOR hunks UN-MASKED + passing 2026-08-07 (scaffold no-op-discard + upstream `+4` editor arg). patchScript now only strips the `script` TTY wrapper and masks the `user@0` edit (user manager) | done |
...
| 26-SYSTEMCTL user@0 edit | Interactive `EDITOR=true`/`EDITOR=mv` edits now RUN + pass (un-masked 2026-08-07: the drop-in scaffold no longer seeds `[Service]` so a no-op discards, and the editor gets the upstream `+4` line arg so `mv +4 <path>` swaps in the prepared file). The `override.conf` `cmp` assertions pass. Remaining: the `user@0` edit line (needs the user manager). The util-linux `script(1)`-under-PID-1 hang is now SIDESTEPPED (patchScript drops the `script` TTY wrapper, which `systemctl edit` does not need) rather than fixed; fixing the parent-side termios/poll bug would let the lines run verbatim with `script` |
