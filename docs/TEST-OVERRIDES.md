...
| 65-ANALYZE | substitute, 1167 upstream lines -> ~184 | de-weakened 2026-08-05: `architectures`, `filesystems`, `syscall-filter` and `transient-settings` added with faithful upstream assertions (VM-validated), joining `verify`/`security`/`condition --unit`/`cat-config`. Still unported: `dump`, `plot` (SVG), and the `--global` flag on `filesystems`/`syscall-filter` (rust rejects it, which the negative assertions exploit). (`architectures --json` IS implemented in `cmd_architectures` and covered here, contrary to an earlier note.) |
...
| 45-TIMEDATE | `testcase_timesyncd` (deep, multi-component). Requires: a networkd-managed dummy iface (`ntp99`, `Kind=dummy`); `timedatectl ntp-servers <link> <servers>` (the port HAS this CLI, `cmd_ntp_servers` in timedatectl); networkd picking up the per-link NTP servers (`assert_networkd_ntp`); and the timesync1 D-Bus interface emitting a `PropertiesChanged` signal for `LinkNTPServers` on change but NOT on a no-op re-set (`assert_timesyncd_signal`, captured via `busctl monitor`). The gap is NOT the CLI but the timesync1 per-link-NTP D-Bus property + PropertiesChanged signal emission plus the networkd link-NTP flow (timesyncd is ~4.6k lines). Multi-cycle, multi-daemon, all-or-nothing (no clean shippable first increment): want user steer, not an autonomous half-start (same class as the mount-ns sandbox and eBPF items). Confirmed structural state: the `Timesync1Manager` zbus interface (`org.freedesktop.timesync1.Manager`, timesyncd/main.rs ~1014) exists with NTPSynchronized/ServerName/ServerAddress/Frequency/poll-interval properties + `Describe()`, but has NO per-link NTP state, no `LinkNTPServers` property, and no `SetRuntimeNTPServers` method, so all of those plus the networkd link-NTP flow (`assert_networkd_ntp`) and the change-only PropertiesChanged emission must be added |
...
| 34-DYNAMICUSERMIGRATE | All four `test_directory` phases (State/Runtime/Cache/Logs) now pass in full, including both `DynamicUser=` directions. `test_check_writable` now PASSES (fixed via the doubly-mounted-root and ProtectSystem=strict corrections; full record in `integration-tests/34-dynamicusermigrate.nix`). The remaining blocker is `test_check_idmapped_mounts`: a missing feature needing id-mapped exec directories (`mount_setattr(MOUNT_ATTR_IDMAP)`) with correct ownership translation (a write must land on-disk as nobody and read back inside as the service's uid). The `create_mapped_userns`/`idmapped_bind` groundwork is in but the translation map is not yet right (12 mechanisms investigated). Instrumenting the exec path (per that guidance) then revealed a PRIOR layer, now FIXED: the service's PrivateUsers uid_map was EMPTY so it ran as nobody, because a DynamicUser two-line self map (`0 0 1` + `<dynuid> <dynuid> 1`) cannot be self-written after `unshare(CLONE_NEWUSER)` any more than a range can; it now goes through the parent-write helper (`unshare_userns_with_helper_written_map`), VM-verified by `07-pid1-dynamicuser-privateusers`. With the map correct, bugs B and C are now FIXED too: (B) the alias exec dirs (`StateDirectory=x:y`) are SYMLINKS to the self-mapped private/ dir, and `move_mount` cannot target a symlink (EINVAL), so `idmapped_bind` is now skipped for symlink alias dests (they already reach the mapped dir); (C) that failed move_mount had left the exec dir broken so the service died before exec, which no longer happens (`07-pid1-dynamicuser-statedir` asserts the service starts and runs). The SOLE remaining blocker is the deepest layer, the mount-idmap ownership DIRECTION: the service's write to its id-mapped StateDirectory still fails (mount idmap `{uid} 0 1` -> EOVERFLOW; upstream's `65534 {uid} 1` + `0 0 1` -> EACCES). The dir is chown'd on-disk to the dynamic uid but the file must land as nobody, which a single mount idmap on that dir cannot obviously satisfy; needs the note's `make_userns` shape reconciled with the now-correct service self-map (4 map shapes tried historically, 2 more here). A DynamicUser+PrivateUsers service WITHOUT StateDirectory runs correctly as its dynamic uid. The wedge that first blocked this family was NOT a race or lock starvation: `systemctl start --wait` polled for `Stopped`, while a completed `Type=oneshot` deliberately stays `Started` to avoid boot activation-graph races, so `--wait` on any oneshot could never return |
