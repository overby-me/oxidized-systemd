...
| 07-PID1 delegate-namespaces | FAILS. Diagnosed: `DelegateNamespaces=` is unparsed, and the mechanism is a deep rework, not a bounded add. systemd (exec-invoke.c `setup_delegated_namespaces`) creates the USER namespace FIRST and the delegated mnt/net/pid/uts namespaces UNDER it (owned by it) so the service, uid 0 in its user ns, has CAP_SYS_ADMIN to manage them (e.g. bind-mount under DelegateNamespaces=mnt), with a multi-pass `have_cap_sys_admin` deferral (mnt is deferred when pid/cgroup/net are delegated). The port creates the user ns LAST (exec_helper.rs ~2923, after mount/net/ipc, deliberately ordered for JoinsNamespaceOf=/PrivateUsers=), so a faithful port must reorder namespace setup, high-regression against working PrivateUsers/PrivateMounts. It is also all-or-nothing over 7 diverse testcases including testcase_user_manager (needs `systemctl start user@0` + `systemd-run --machine --user`) and testcase_multiple_features (needs the minimal_0.raw squashfs). A poor single-target; better tackled as its own multi-step effort. |
