...
| 07-PID1 concurrency | PARTIAL, still FAILS (not skipped, no false pass). `ConcurrencyHardMax=`/`ConcurrencySoftMax=` are now parsed (SliceConfig) and enforced: a new `slice_concurrency` module ports systemd's `slice_get_currently_active` and the soft/hard reached checks (counting a slice subtree recursively, sub-slices included, walking up parents), the hard limit refuses the start at enqueue, and the soft limit parks the start (keeping its job Waiting and blocking the client) until a member stops. This carries the test through its whole first section (soft-block of the 4th unit → `timeout 1s`→124, release on stop, hard-refusal of the 5th). Two gaps remain before it greens, both scoped to concurrency-limited slices so nothing else is affected (01-basic green): (1) the soft-limit RELEASE is polled, not event-driven, so it races the test's synchronous `stop; start sync; is-active` sequence (a faithful version needs the exit handler to re-dispatch queued starts synchronously, and `After=` to wait on a pending job); (2) the NESTED "two slots (slice+service)" case additionally needs `systemctl stop <slice>` to mark the slice unit Stopped so it stops counting. |
...
| 07-PID1 private-pids | substitute; procfs check strengthened 2026-08-05 | Biggest single coverage loss in the 07 family. The procfs-mount assertion now checks the full `rw,nosuid,nodev,noexec` option set (was `nosuid` only), VM-verified. NOT added, and found as a real bug while trying: upstream testcase_basic SIGKILLs the namespace PID 1 and asserts `Result=signal`/`ExecMainStatus=9`, but rust records a SIGKILLed `PrivatePIDs=yes` (CLONE_NEWPID) main as `Result=exit-code` -- an exit-status-handling gap for the PID-namespace child, deferred. Still substitutes the rest (testcase_analyze `systemd-analyze verify`, testcase_multiple_features, testcase_unpriv which needs the user manager + `/home/testuser` blocked by #35) |
