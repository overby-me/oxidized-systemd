...
This document gives a overview of how information about sockets are opened and passed to services / activate services in systemd-rs.
...
systemd-rs has a 'select' waiting on all FDs that are not currently passed to a service. If one of them has data read the respective service is activated (and the possibility to ignore the activation is disabled)
