...
* Stop unit with that name. Will recursively stop all units that require that unit
...
* Load new file with those name(s). Useful if you moved/copied a file in the unit-dirs and want to start it without restarting systemd-rs as a whole.
...
* Shutdown systemd-rs by killing all services, closing all sockets and exiting
...
Alternatively you can use something like socat to send commands or whatever you'd like. (There is a need for a better userinterface though PRs very welcome!)
