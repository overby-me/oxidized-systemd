...
This document gives a overview of how information about pids is passed around in systemd-rs
...
These entries tie service-ids and pids together so systemd-rs knows which service a process belonged to
...
1. If it was a service check if it was in the 'Started' state (if not it was killed by another systemd-rs mechanism)
...
this pid to change from Helper(...) to HelperExited(...). This could be improved by using a condition variable that
