...
Currently systemd-rs is still pretty spaghetti, since I just started writing code to see what components are needed, and wanted to play around with this project. Now that pretty much all concepts work(tm) the existing bugs are likely caused by the bad (read: missing) design.
...
1. Locking. Mutexes are something I first started using when starting systemd-rs so I used them all over the place and by that enabled deadlocking to happen. This should possible to work around by making rules in which order stuff has to be locked.
1. Updating the set of units needs to happen 'atomically' so we need to be able to lock the whole runtimeinfo
1. Make finding units and their inter-dependencies easier. It is currently very annoying and verbose to find all units that need each other either by name or implicit dependency. This probably means systemd-rs should keep track of the name-dependencies when adding/removing units.
...
1. Dont use u64 for IDs. Just use the names of the units, those have to be unique anyways. And the performance of comparing IDs shouldn't really matter. This is
    an obvious case of premature optimization that made the code and tracing unnecessarily weird.
1. The UnitStatus should be refactored into multiple enums to cleanly model the reasons why a unit is currently not running
    ...
    1. Stopped (The unit has been stopped in some way)
        1. The reason should be represented with it's own enum:
            1. Unexpected (Only possible for services, when the main process exited for a reason outside of systemd-rs
        ...
        1. There might have occurred an error while starting
            1. PreStartError
            1. StartError
            1. PostStartError
        1. There might have occurred an error while stopping
            1. StopError
            1. PostStopError
...
1. systemd-rs knows one RwLock<UnitSet>.
...
Events are what systemd-rs should work on. Currently it only does so after the initial startup, which is bad because it duplicates a lot of code. And
only the initial startup does actually start units in parallel.
...
Initially systemd-rs has a set of inactive units. Then a command will be emulated that starts the configured target unit (default.target in most cases).
...
The whole handling of services in systemd-rs as a parent-process is pretty solid.
...
1. have a cli flag for systemd-rs that tells it it is in child mode an do the necessary setup + execing again
