...
**🟢 NixOS boots successfully with systemd-rs as PID 1** — reaches `multi-user.target` with login prompt in ~8 seconds (cloud-hypervisor VM, full networking via networkd + resolved). **5,762 unit tests passing** across 68 crates.

| Phase | Status |
|-------|--------|
| Phase 0 — Foundation | ✅ Complete |
| Phase 1 — Core System | ✅ Complete (path units, sd_notify full protocol: FDSTORE/FDSTOREREMOVE/FDNAME/ERRNO/BUSERROR/EXIT_STATUS/MONOTONIC_USEC/INVOCATION_ID/WATCHDOG_USEC dynamic) |
| Phase 2 — Essential System Services | 🔶 In progress (udevd partial — worker thread pool done, logind/hostnamed/timedated/localed D-Bus done, OnCalendar= full parser done, networkd/resolved D-Bus done) |
| Phase 3 — Network Stack | 🔶 Partial (networkd ✅ D-Bus + `.link` parsing, resolved ✅ D-Bus + DNS cache, timesyncd ✅ D-Bus, timedated ✅ D-Bus, hostnamed ✅ D-Bus, localed ✅ D-Bus, networkd-wait-online ✅) |
| Phase 4 — Extended Services | 🔶 Partial (machined ✅ D-Bus, portabled ✅ D-Bus, homed ✅ D-Bus, oomd, coredump, sysext, dissect, firstboot, creds) |
...
- ✅ **`systemd` (PID 1)** — service manager with all core unit types (service, socket, target, mount, timer, path, slice, scope) and all service types (`simple`, `exec`, `notify`, `notify-reload`, `oneshot`, `forking`, `dbus`, `idle`), default target handling, parallel activation, fstab generator, getty generator, NixOS early boot setup, full `Condition*`/`Assert*` directive support (15 check types), proper `Type=idle` deferral (idle services wait for all other jobs to complete before starting); **path unit support** with `PathExists=`, `PathExistsGlob=`, `PathChanged=`, `PathModified=`, `DirectoryNotEmpty=`, `MakeDirectory=`, `DirectoryMode=`, `TriggerLimitIntervalSec=`, `TriggerLimitBurst=`, `Unit=` — poll-based filesystem monitoring with change detection, glob matching, rate limiting, and `MakeDirectory=` support; **full sd_notify protocol**: `MAINPID=` updates tracked PID, `WATCHDOG=1` pings recorded, `WATCHDOG=trigger` immediate action, `RELOADING=1`/`STOPPING=1` state tracking with `READY=1` reload-complete detection, `WATCHDOG_USEC=` dynamic timeout override, `ERRNO=`/`BUSERROR=`/`EXIT_STATUS=` error reporting stored for status display, `MONOTONIC_USEC=`/`INVOCATION_ID=` metadata, **`FDSTORE=1`/`FDSTOREREMOVE=1`/`FDNAME=`** fd store with SCM_RIGHTS via `recvmsg()` (respects `FileDescriptorStoreMax=`, passes stored FDs back on restart via `LISTEN_FDS`/`LISTEN_FDNAMES`); missing: emergency/rescue mode, external generators, transient units, reexecution, `SIGRTMIN+` signals
...
- 🔶 **Comprehensive test suite** — 5,762 unit tests passing; integration tests via nixos-rs boot test; missing: differential testing against real systemd
