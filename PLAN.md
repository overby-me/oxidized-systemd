...
Merged into `main` on 2026-07-16 (the historical `systemd-test` bookmark);
further work happens on `main` via feature bookmarks.
...
### 1. D-Bus Interface (org.freedesktop.systemd1) — IMPLEMENTED
...
### 5. Exec Deserialization — DONE

- 07-pid1-exec-deserialization: ExecStart commands added after daemon-reload
  are now picked up during a running oneshot (exec index tracking across
  daemon-reload landed; see the status summary above).

### 6. udev Tests — all 22 subtests pass

The Rust udevadm reimplementation covers the full 17-udev suite; the
subsections below record how each area landed.
