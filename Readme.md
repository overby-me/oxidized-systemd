...
systemd-rs is a service manager that tries to replicate systemd behaviour for a subset of the configuration possibilities. It focuses on the core functionality of a service manager, not requiring to be PID1 (aka init process).

## Will this replace systemd?

TLDR: No, systemd-rs is no dedicated replacement. It is an opportunity for the niches where systemd could not get it's foot down to profit (more easily) from the
ecosystem around systemd.

Very likely no. There are a lot of reasons, but most importantly: it works and provides features that systemd-rs will likely never provide.
...
reject using systemd-rs as their main service manager. But having systemd-rs as an example might help other efforts in more portable languages.

systemd-rs also opens up usage of systemd services outside of systemd based linux distros (like alpine linux, commonly used in docker containers and small vms) and
freebsd.
...
It does look somewhat promising, most needed features are there. There are a lot of tests missing and more care needs to be taken so that systemd-rs itself never panics.

### Short intro to systemd / systemd-rs

Systemd/systemd-rs operate on so called "units". These are smallish separate entities in the system like a single service. These units can be handled independently but
...
The second thing systemd/systemd-rs bring to the table is socket activation. Services that specify sockets do not need to be started immediately, but rather when there is
activity on their socket(s). This enables faster startup times by starting services lazily when they are needed.

Additionally systemd provides a lot more unit-types besides services and sockets which systemd-rs does not (and for most will likely never) support.
...
1. Scopes (Nope. If you start processes outside of systemd-rs you need to manage them yourself. Maybe a second instance of systemd-rs? ;))
1. Slices (this might be added as it is fairly important if you are not running inside of a container)

[![Gitter](https://badges.gitter.im/systemd-rs/community.svg)](https://gitter.im/systemd-rs/community?utm_source=badge&utm_medium=badge&utm_campaign=pr-badge)
...
1. systemd-rs doesnt have to concern itself with how a platform does resource restriction, but there can be separate tools for each platform (if possible)
1. Clear separation of concerns. systemd-rs manages service lifetimes. It does not (or only for relatively trivial stuff) manage the runtime environment for those services.
...
Here is a list of features systemd-rs currently assumes the platform to provide. The list also contains suggestions about which features
...
systemd-rs can employ cgroups for better control over which processes belong to which service. Resource-limiting is still out of scope for systemd-rs. Cgroups
are only used to make the features systemd-rs provides anyways more reliable.
...
One possibility would be to use BSD jails but that seems somewhat hacky since systemd-rs would have to chain-load the actual service command with a jail command.
systemd-rs could check if a service is started in a jail anyways and then kill that jail. But that could lead to other problems if the jail is meant to be long lived.
...
For an in-depth comparison of systemd and systemd-rs see the feature-comparison.md file. It is generated by the tools/gen_feature_comparison.py (shoutout to [wmanley](https://github.com/wmanley) who wrote the initial script!).
It currently is somewhat pessimistic, I will work on improving the comparison of the features systemd-rs actually does support (see below for a list of supported features).

### General features

Of systemd-rs itself
...
* Matching services and sockets either by name or dynamically by parsing the appropriate settings in the .service/.socket files
...
* systemd-rs (stripped binary built with musl to be completely static) -> ~1.6Mb
...
Running `./build_all.sh && cargo run --bin systemd-rs` will build the test services and run systemd-rs which will start them.
...
* An optional journald logging. (Maybe thats not something that is actually something that is wanted)
    ...
    1. Negative: Weird dependency between systemd-rs and a service managed by systemd-rs (could be less of a pain point if systemd-rs itself handled logging in a journald way)
...
* Service type forking is missing
  * I would argue that this is an unnecessary type anyways. This would be better handled by using something like
...
* A systemctl equivalent to control/query systemd-rs (there is a small jsonrpc2 API but that might change again)
...
systemd-rs has two binaries: The main service-manager 'systemd-rs' and the control client 'rsdctl'.

The client is just a dumb utility to
pack cli arguments into jsonrpc2 format and send them to systemd-rs. This can be used to restart units, add new units, or show the status
of units.

Generally systemd-rs has two phases:
...
There has been a request for a place to talk about this project, so I opened a gitter community for this project. Feel free to come over and have a chat [on this page](https://gitter.im/systemd-rs/community?utm_source=share-link&utm_medium=link&utm_campaign=share-link)
